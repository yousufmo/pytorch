...
Tensor _clamp(
  ...
  if (v_self.is_quantized()) {
    float mini = min ? roundevenf(min->to<float>() / v_self.get_scale()) +
            v_self.get_zero_point()
                     : -std::numeric_limits<float>::infinity();
    float maxi = max ? roundevenf(max->to<float>() / v_self.get_scale()) +
            v_self.get_zero_point()
                     : std::numeric_limits<float>::infinity();
