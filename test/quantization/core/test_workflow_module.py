...
class TestObserver(QuantizationTestCase):
    ...
    def test_histogram_observer_handle_OOM_due_to_close_min_max_value(self):
        obser = HistogramObserver.with_args(reduce_range=False)()
        # close min and max value in the 1st forward() pass of observer tends to cause OOM in the following pass due to the allocation of histogram tensor during _combine_histograms(). After adding try-catch clause, we expect the histogram observer can still work by resetting the histogram
    ...
    def test_histogram_observer_handle_OOM_due_to_large_upsample_rate(self):
        # a large upsample rate leads to OOM due to the allocation of histogram tensor during _combine_histograms(). After adding try-catch clause, we expect the histogram observer can still work by resetting the histogram
